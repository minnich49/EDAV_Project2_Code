...
---
##Introduction 
For this report we will be diving into the global issue of Flood Events. The initial goal of the paper will be to investigate the datasets to better understand. The dataset being investigated involves geographical, spatial, and time parameters that create an extremely large amount of information to be explored. We will work to break down this large dataset into something smaller and begin to focus on specific events within the flood dataset.
...
setwd("/Users/jiabinchen/Desktop/W4701/proj2/EDAV_Project2_Code-master_2")
```
...
```
An interactive google map with a portion of tagged flood locations (1985-present) is displayed below. This feature allows the user to zoom in and see exactly where a flood event occurred. Clicking the tag shows the date of the event.
...
## Top 10 Country Investigation
  Now we will investigate the top 10 countries within different categories of the flood data. To see if we can visualize any unique characteristics within the data, we will look at variables such as the number of floods, deaths by the floods and people displaced by the floods.
...
We can see that there are some insights within some of these plots. We can see that there are several countries that appear in all three plots. These are Brazil, China, India, Indonesia, Philippines, USA and Vietnam. Seeing some of the more developed countries such as the USA seems surprising here because of how many flood events actually occur. There could be a difference in reporting here as well, such as the USA has been recording these events longer and therefore has a higher number of events.


Another interesting finding is that Iran, Pakistan and Afghanistan appear in the highest number of deaths but not in the highest number of displaced. Looking deeper into these categories we find that they just most likely missed the top 10.
...
So we can see a positive correlation occurring between the number of Flood Events, Deaths and Displaced.
...
We then continued looking into the differences between northern and southern hemispheres looking at density plots of various attributes of the flood events.
...
These density plots show us that the total number of flood events occurred between the years of 2002 - 2003 by looking at the left column of plots "Total Time Period" Looking at  "Differences between Hemispheres" we can actually see this may be mostly contributed by the Southern Hemisphere Countries beginning to record their Flood Events along with the countries in the Northern Hemisphere. We can see that from 2002 on the Southern Hemisphere makes up more of the recorded events.

From the same column we can look at the Severity Plots for the Total Time Period. The Middle Severity of 1.5 does not start until 2005. This suggests that the rating for this category did not previously exist, judging by how much it is currently used.

We then began to look for correlations between elevation and different aspects of the flooding events. The general intuition would be that the higher the elevation, the lower the number of flood events

As we look at the right side of the plots "Grouped by Month" we can further see some interesting characteristics. Looking at the upper right plot it would seem that for the world most of the flooding occurs in the 7th month, July. However as we break this down to Northern and Southern Hemispheres we can see that these seasons are opposite for each region. This makes sense because the different hemispheres experience opposite seasons.
...
An interesting finding from this graph is the outlier that was located at the top for Duration in Days. We can see that there is a flood event that happened in the USA and lasted for 419 days. The next closest was 168 days which occurred in Zambia. We can also see a negative correlation with the elevation and duration in days. This was removed in the plotting due to resolution.
...
For Affected Square km it might seem that there is no correlation but there is a slightly negative correlation between the Elevation and Affected Square km. the Y-axis is suppressing this but is not a strong enough correlation to be relevant.
...
library(ncdf4)
library(RNetCDF)
setwd("/Users/jiabinchen/Desktop/W4701/proj2/")
...
<p>The following plot is an interactive map that will allow you to search through the floods that occurred in 2013, where the area of the circle surrounding each point will tell you how large the affected radius of each event is. Click on any event to find additional information including country, start date, end date, duration, deaths, displacement and damage in US dollars if the information is available.</p>
...
There is one Principle Component that explains quite a lot of the variance. The left PCs drop quickly after first one and decrease accordingly.
...
The plot shows, except two centroid, the other 8 factors have similar directions and they can be explained by first principle component well. Among the groups of the main causes of floods, we can see "Snow Melt" and "Monsoonal rain" typically lie more right, which indicates heavier damage. While "Ice Jams" and "Brief torrential rain" are left in the plot, which indicates less damage.
...
```
Clearly, the most floods occurred in February and there was at least one flooding event in every month. Next, we examine the data describing the damage caused by floods in the USA.
...
There are also other factors to consider such as rivers, water shelves and lakes around the area. It could be that there was heavy rain in other areas that caused downstream flooding of rivers. These visuals do seem to point to flooding as a result of change in pressure but more investigation is required.
